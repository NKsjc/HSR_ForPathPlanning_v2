...
*/
//		for(int j=0;j<4;j++)ref_value[j]=0;//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		}
	}
}
...
double P2PTrajectoryPlan::Get_Hand_Trajectory(hand_trajectory_info &sul)//»ñÈ¡¹ìŒ£¹æ»®µÄµã
{
	...
	//ŒõŒÓËÙœ×¶Î
	if (t>=sul.l_t[0]+sul.l_t[1]&&t<sul.l_t[0]+sul.l_t[1]+sul.l_t[2])
	{
		value=sul.l_a2*(t - sul.l_t[0] - sul.l_t[1]) + (sul.l_k[1]*pow(t-sul.l_t[0] - sul.l_t[1],2))/2. + sul.l_v[1];
	}
	//ÔÈËÙœ×¶Î
	if (t>=sul.l_t[0]+sul.l_t[1]+sul.l_t[2]&&t<sul.l_t[0]+sul.l_t[1]+sul.l_t[2]+sul.l_t[3])
	{
		value=sul.l_v[2];
	}
	//ŒÓŒõËÙœ×¶Î
	if (t>=sul.l_t[0]+sul.l_t[1]+sul.l_t[2]+sul.l_t[3]&&t<sul.l_t[0]+sul.l_t[1]+sul.l_t[2]+sul.l_t[3]+sul.l_t[4])
	{
		value=(sul.l_k[2]*pow(t -sul.l_t[0] - sul.l_t[1] - sul.l_t[2] - sul.l_t[3],2))/2. +
			(t -sul.l_t[0] - sul.l_t[1] - sul.l_t[2] - sul.l_t[3])*(sul.l_a6- sul.l_k[2]*sul.l_t[4]) + sul.l_v[3];
	}
	//ÔÈŒõËÙœ×¶Î
	if (t>=sul.l_t[0]+sul.l_t[1]+sul.l_t[2]+sul.l_t[3]+sul.l_t[4]&&t<sul.l_t[0]+sul.l_t[1]+sul.l_t[2]+sul.l_t[3]+sul.l_t[4]+sul.l_t[5])
	{
		value=sul.l_v[4] + sul.l_a6*(t -sul.l_t[0]-sul.l_t[1]-sul.l_t[2]-sul.l_t[3]-sul.l_t[4]);
	}
	//ŒõŒõËÙœ×¶Î
	if (t>=sul.l_t[0]+sul.l_t[1]+sul.l_t[2]+sul.l_t[3]+sul.l_t[4]+sul.l_t[5]&&t<=sul.l_t[0]+sul.l_t[1]+sul.l_t[2]+sul.l_t[3]+sul.l_t[4]+sul.l_t[5]+sul.l_t[6])
	{
		value=sul.l_a6*(t - (sul.l_t[0]+sul.l_t[1]+sul.l_t[2]+sul.l_t[3]+sul.l_t[4]+sul.l_t[5]) )
			+ (sul.l_k[3]*pow(t - (sul.l_t[0]+sul.l_t[1]+sul.l_t[2]+sul.l_t[3]+sul.l_t[4]+sul.l_t[5]) ,2))/2. + sul.l_v[5];
	}
	return value;
...
int P2PTrajectoryPlan::Trajectory_From_Target(double st_b[4], double st_e[4],double w_b[4],double w_e[4])
{
	...
	for(i=0;i<4;i++)
	{
		hsrlog.Print(_Debug,"begin trajectory of joint %d  ,the input is current: %4ftarget:   %4f\n",i+1,st_b[i],st_e[i]);
		res=this->Cal_Trajectory(st_b[i],st_e[i],w_b[i],w_e[i],this->hand_tra[i]);
	//	res=0;
	//	this->hand_tra[i].Tmax=3;
		if(res!=0)
		{
	...
	for(i=0;i<4;i++){
		hsrlog.Print(_Debug,"the trajectory of joint %d is ok,the time is %4f\n",i+1,this->hand_tra[i].Tmax);
